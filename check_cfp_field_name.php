...
if($_POST['prev']!='new')
{
	$qry = "select count(*) from $cfp_fields where Name ='".$_POST['name']."' and Name !='".$_POST['prev']."' and Form_Id=".$_POST['formid'];
...
{
	$qry = "select count(*) from $cfp_fields where Name ='".$_POST['name']."' and Form_Id=".$_POST['formid'];
	$result = $wpdb->get_var($qry);
	if($result!=0)
	{
		echo '<div style=" color:red">'.__('Warning! Field label already exists. Please choose a unique label.',$textdomain ).'</div>';	
