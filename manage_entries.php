...
{
	$qry = "select id from $cfp_forms order by id asc limit 1";
...
if(!empty($_POST['selected']) && isset($_POST['remove']))
{
	...
	if (!wp_verify_nonce($retrieved_nonce, 'manage_cfp_entries' ) ) die( 'Failed security check' );

	$ids = implode(',',$_POST['selected']);
	$query = "delete from $cfp_entries where id in($ids)";
	$wpdb->get_results($query);
}
?>

<form name="field_list" id="field_list" method="post">
<?php wp_nonce_field('manage_cfp_entries'); ?>
  <div class="cfp-main-form">
    <div class="cfp-form-name-heading-Submissions">
      <h1><?php _e('Submissions', $textdomain ); ?></h1>
    </div>
    <div  class="cfp-add-remove-field-submissions cfp-new-buttons">
      <div class="cfp-remove-field" style="display:none;padding-top: 2px;">
...
if($_REQUEST['form_id']!="")
{
  ...
  $entries = $wpdb->get_results( "SELECT * FROM $cfp_entries where form_id ='".$form_id."' order by id desc LIMIT $offset, $limit");
}
...
?>
      <li class="header rows">
        ...
        <div class="cols" style="width:30px;">#</div>
        <?php
	$qry = "select * from $cfp_fields where form_id ='".$_REQUEST['form_id']."' and Type not in('heading','paragraph','file') order by ordering asc";
	$reg = $wpdb->get_results($qry);
...
echo $fieldnamehalf.'...';
  ...
  ?>
      <li class="<?php echo $class;?> rows">
        ...
        <div class="cols" style="width:30px;"><a href="admin.php?page=cfp_view_entry&id=<?php echo $entry->id;?>"><?php echo $i; ?></a></div>
        <?php
	 $field =1;
	...
	$field++;
	 ...
	 ?>
     <div class="cols" style="width:50px"><a href="admin.php?page=cfp_view_entry&id=<?php echo $entry->id;?>">View</a></div>
      </li>
      <?php 
$i++;
} 
...
if ( $page_links ) {
    echo '<div class="tablenav cfppagination"><div class="tablenav-pages" style="margin: 1em 0">' . $page_links . '</div></div>';
}
?>
  </div>
</form>
<script>
    ...
    jQuery('input[name="selected[]"]').click(function () {
        ...
        if (atLeastOneIsChecked == true) {
            jQuery('.cfp-remove-field').show(500);
        } else {
            jQuery('.cfp-remove-field').hide(500);
        }
    });
</script>
<script>
